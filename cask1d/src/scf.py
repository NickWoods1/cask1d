...
def pulay_mixing2(params, density_differences, residual_differences, current_residual, current_density_in, i, precond):
    r"""
    ...
    density_in = current_density_in + params.step_length*np.dot(precond, current_residual)
    for j in range(0,history_size):
        density_in[:] += pulay_coefficients[j]*(density_differences[j,:] + params.step_length*np.dot(precond, residual_differences[j,:]))
